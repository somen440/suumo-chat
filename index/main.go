...
func newApp(projectID, templateDir string) (*app, error) {
	...
	tmpl, err := template.New(base).Funcs(template.FuncMap{
		"formatToAsia": func(dt time.Time) string {
			return dt.Add(9 * time.Hour).Format("2006-01-02 15-04-05")
		},
	}).ParseFiles(path)
	if err != nil {
...
func (a *app) index(w http.ResponseWriter, r *http.Request) {
	docs, err := a.firestoreClient.Collection("translations").OrderBy("CreatedAt", firestore.Desc).Limit(10).Documents(r.Context()).GetAll()
	if err != nil {
    ...
    translations := make([]background.Translation, len(docs))
    for i, j := len(docs) - 1, 0; i >= 0; i-- {
...
func formatToAsia(dt time.Time) string {
	return dt.Add(9 * time.Hour).Format("2006-01-02 15-04-05")
}
